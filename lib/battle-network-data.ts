...
export type BattleNetworkInitialData = { agencies: ReturnType<typeof toAgency>[]; battles: ReturnType<typeof toBattle>[]; cardLayout?: unknown; cardTypography?: unknown; managerSettings?: unknown; incompatibilities?: unknown; bannedCreators?: unknown; visibilityBlocks?: unknown; posterMade?: unknown; error?: string };
...
export async function getBattleNetworkInitialData(): Promise<BattleNetworkInitialData> {
  ...
  const settings = (settingsResult.data?.template_json || {}) as { cardLayout?: unknown; cardTypography?: unknown; managerSettings?: unknown; incompatibilities?: unknown; bannedCreators?: unknown; visibilityBlocks?: unknown; posterMade?: unknown };
  console.info(`[battle-network] initial parallel queries ${Math.round(performance.now() - startedAt)}ms`);
  return { agencies: (agenciesResult.data || []).map(toAgency), battles: (battlesResult.data || []).map(toBattle), cardLayout: settings.cardLayout, cardTypography: settings.cardTypography, managerSettings: settings.managerSettings, incompatibilities: settings.incompatibilities, bannedCreators: settings.bannedCreators, visibilityBlocks: settings.visibilityBlocks, posterMade: settings.posterMade };
}

...
type TwoVTwoParticipant = { username: string; agencyId: string };
const twoVTwoParticipants = (value: unknown): TwoVTwoParticipant[] | null => {
  try {
    ...
    const participants = parsed.map((item) => ({ username: clean(item?.username), agencyId: key(item?.agencyId) }));
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "make-internal-agency") { const id = key(body.agencyId); const { data, error } = await submissionsSupabase.from("battle_network_agencies").update({ name: clean(body.name).toUpperCase(), password: clean(body.password).toUpperCase(), logo_url: String(body.logoUrl || ""), external_only: false, accent: body.accent || "#d4af37" }).eq("id", id).select(AGENCY_COLUMNS).single(); if (error || !data) throw new Error(error?.message || "AGENCY NOT FOUND."); return NextResponse.json({ agency: toAgency(data) }); }

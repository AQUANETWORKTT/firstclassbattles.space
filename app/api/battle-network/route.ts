...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "match") { const [first, second, currentSettings] = await Promise.all([battle(String(body.firstId)), battle(String(body.secondId)), settings()]); const close = Boolean(body.close); const activeAgencyId = key(body.agencyId); const minutes = (value: string) => { const [hours, mins] = value.split(":").map(Number); return hours * 60 + mins; }; const difference = first && second ? Math.abs(minutes(first.requestedTime) - minutes(second.requestedTime)) : 0; if (!first || !second || first.opponentBattleId || second.opponentBattleId) return NextResponse.json({ error: "THAT BATTLE IS NO LONGER AVAILABLE." }, { status: 409 }); if (!activeAgencyId || !battleBelongsToAgency(first, activeAgencyId)) return NextResponse.json({ error: "YOU CAN ONLY MATCH FROM YOUR OWN AGENCY BATTLE." }, { status: 403 }); if (battleBelongsToAgency(second, activeAgencyId)) return NextResponse.json({ error: "YOU CANNOT MATCH TWO BATTLES THAT BOTH BELONG TO YOUR AGENCY." }, { status: 409 }); if (first.agencyId === second.agencyId) return NextResponse.json({ error: "AN AGENCY CANNOT MATCH AGAINST ITS OWN BATTLE." }, { status: 409 }); if (Boolean(twoVTwoParticipants(first.manager)) !== Boolean(twoVTwoParticipants(second.manager))) return NextResponse.json({ error: "2V2 BATTLES CAN ONLY BE MATCHED WITH ANOTHER COMPLETE 2V2 PAIR." }, { status: 409 }); if (creatorKey(first.creatorUsername) === creatorKey(second.creatorUsername)) return NextResponse.json({ error: "A CREATOR CANNOT BE MATCHED AGAINST THEMSELVES." }, { status: 409 }); if (incompatible(incompatibilities(currentSettings), first.creatorUsername, second.creatorUsername)) return NextResponse.json({ error: "⚠ INCOMPATIBLE CREATORS — THIS BATTLE CANNOT BE MATCHED." }, { status: 409 }); if (first.day !== second.day || first.size !== second.size || first.powerUps !== second.powerUps || (!close && first.requestedTime !== second.requestedTime) || (close && (difference === 0 || difference > 15 || minutes(first.requestedTime) < 1080 || minutes(second.requestedTime) < 1080))) return NextResponse.json({ error: "BATTLES MUST MATCH ON DAY, TIME, SIZE AND POWER-UP RULES." }, { status: 409 }); const updates = close ? [{ id: first.id, opponent_battle_id: second.id, actual_time: second.requestedTime }, { id: second.id, opponent_battle_id: first.id }] : [{ id: first.id, opponent_battle_id: second.id }, { id: second.id, opponent_battle_id: first.id }]; const results = await Promise.all(updates.map(({ id, ...update }) => submissionsSupabase.from("battle_network_battles").update(update).eq("id", id))); const error = results.find((result) => result.error)?.error; if (error) throw new Error(error.message); return NextResponse.json({ ok: true }); }

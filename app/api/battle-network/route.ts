...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "save-2v2-battle") {
      ...
      const row = payload({ ...battle, agencyId: key(battle.agencyId) || participants[0].agencyId, creatorUsername: participants[0].username, manager: `${TWO_V_TWO_PREFIX}${JSON.stringify(participants)}`, powerUps: "NPU" });

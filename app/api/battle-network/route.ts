...
const visibilityBlocks = (value: unknown): VisibilityBlock[] => Array.isArray(value) ? value.map((item) => ({ viewerAgencyId: key(item?.viewerAgencyId), targetAgencyId: key(item?.targetAgencyId) })).filter((item) => item.viewerAgencyId && item.targetAgencyId && item.viewerAgencyId !== item.targetAgencyId) : [];
const isVisibilityBlocked = (blocks: VisibilityBlock[], viewerAgencyId: string, targetAgencyId: string) => blocks.some((block) => block.viewerAgencyId === viewerAgencyId && block.targetAgencyId === targetAgencyId);
async function hasFirstClassAdminPassword(password: unknown) { const supplied = String(password || "").trim().toUpperCase(); if (!supplied) return false; if (MASTER_PASSWORDS.has(supplied)) return true; const { data, error } = await submissionsSupabase.from("battle_network_agencies").select("password").eq("id", "first-class-dan-james").maybeSingle(); if (error) throw new Error(error.message); return String(data?.password || "").trim().toUpperCase() === supplied; }
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!results) return NextResponse.json({ agencies: [], battles: [], cardLayout: layout.cardLayout, cardTypography: layout.cardTypography, managerSettings: layout.managerSettings || {}, weeklySchedules: weeklySchedules(layout), incompatibilities: incompatibilities(layout), bannedCreators: Array.isArray(layout.bannedCreators) ? layout.bannedCreators : [], visibilityBlocks: visibilityBlocks(layout.visibilityBlocks), posterMade: layout.posterMade || {} });
...
return NextResponse.json({ ok: true }); }
    ...
    const linkedOpponent = source?.opponentBattleId ? await battle(source.opponentBattleId) : null;
    if (source?.opponentBattleId && (body.action === "add-manual-opponent" || !linkedOpponent || linkedOpponent.cancelledAt || linkedOpponent.opponentBattleId !== source.id)) {
      const repairs = [submissionsSupabase.from("battle_network_battles").update({ opponent_battle_id: null, actual_time: source.requestedTime, cancelled_at: null, cancelled_by: null }).eq("id", source.id)];
      if (linkedOpponent?.opponentBattleId === source.id) repairs.push(submissionsSupabase.from("battle_network_battles").update({ opponent_battle_id: null, actual_time: linkedOpponent.requestedTime, cancelled_at: null, cancelled_by: null }).eq("id", linkedOpponent.id));
      const repairError = (await Promise.all(repairs)).find((result) => result.error)?.error;
    ...
    if (["claim-battle", "external-claim", "add-manual-opponent"].includes(body.action)) {
      ...
      if (body.action === "add-manual-opponent" && source && agency && username) {
        const { data: sameSlot, error: sameSlotError } = await submissionsSupabase.from("battle_network_battles").select(BATTLE_COLUMNS).eq("agency_id", agencyId).eq("week_start", source.weekStart).eq("day", source.day).eq("requested_time", requestedTime);
        if (sameSlotError) throw new Error(sameSlotError.message);
        const existing = (sameSlot || []).map(toBattle).find((row) => creatorKey(row.creatorUsername) === creatorKey(username));
        const existingOpponent = existing?.opponentBattleId ? await battle(existing.opponentBattleId) : null;
        const existingIsStale = existing && (!existing.opponentBattleId || !existingOpponent || existingOpponent.cancelledAt || existingOpponent.opponentBattleId !== existing.id);
        if (existingIsStale) {
          const repairs = [
            submissionsSupabase.from("battle_network_battles").update({ opponent_battle_id: existing.id, requested_time: requestedTime, actual_time: requestedTime }).eq("id", source.id),
            submissionsSupabase.from("battle_network_battles").update({ opponent_battle_id: source.id, actual_time: requestedTime, cancelled_at: null, cancelled_by: null }).eq("id", existing.id),
          ];
          if (existingOpponent?.opponentBattleId === existing.id) repairs.push(submissionsSupabase.from("battle_network_battles").update({ opponent_battle_id: null, actual_time: existingOpponent.requestedTime, cancelled_at: null, cancelled_by: null }).eq("id", existingOpponent.id));
          const repairError = (await Promise.all(repairs)).find((result) => result.error)?.error;
          if (repairError) throw new Error(repairError.message);
          return NextResponse.json({ battle: { ...existing, opponentBattleId: source.id, actualTime: requestedTime }, agency: toAgency(agency) });
        }
      }
      if (agencyError) throw new Error(agencyError.message); if (!agency || !username) return NextResponse.json({ error: "COMPLETE THE OPPONENT DETAILS." }, { status: 409 }); const currentSettings = await settings(); if (isVisibilityBlocked(visibilityBlocks(currentSettings.visibilityBlocks), agencyId, source.agencyId)) return NextResponse.json({ error: "THIS AGENCY IS BLOCKED FROM VIEWING OR PAIRING THIS BATTLE." }, { status: 403 }); if (rejectBanned(currentSettings, source.creatorUsername, username)) return NextResponse.json({ error: "A BANNED CREATOR CANNOT BE ADDED TO A BATTLE." }, { status: 403 }); if (incompatible(incompatibilities(currentSettings), source.creatorUsername, username)) return NextResponse.json({ error: "⚠ INCOMPATIBLE CREATORS — THIS BATTLE CANNOT BE CLAIMED." }, { status: 409 }); if (await hasDuplicateBattle({ agency_id: agencyId, week_start: source.weekStart, day: source.day, creator_username: username, requested_time: requestedTime })) return NextResponse.json({ error: "A BATTLE FOR THIS CREATOR AT THIS TIME ALREADY EXISTS." }, { status: 409 });
